...
const PureChatItem = ({
  ...
  console.log('onUpdateTitle:', onUpdateTitle);
  ...
  return (
    <SidebarMenuItem>
      ...
      {showRenameDialog && (
        <div className="fixed inset-0 flex items-center justify-center bg-zinc-900 bg-opacity-82 z-50">
          <div className="bg-black p-4 rounded-lg shadow-lg w-96">
            <h2 className="text-lg font-bold mb-2">Rename Chat</h2>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button
                ...
                className="px-4 py-2 bg-red-950 rounded"
              >
                Cancel
              </button>
              <button
...
export function SidebarHistory({ user }: { user: User | undefined }) {
  ...
  const handleUpdateTitle = async (chatId: string, newTitle: string) => {
    ...
    mutate((history) =>
      ...
      history
        ? history.map((c) => (c.id === chatId ? { ...c, title: newTitle } : c))
        : []
